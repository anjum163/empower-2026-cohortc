...
def addend(sum_expression: Expression) -> Expression:
  ...
  raise ValueError("Not a sum expression")
...
def multiplier(product_expression: Expression) -> Expression:
  ...
  raise ValueError("Not a product expression")
...
def derivative(expression: Expression, variable_name: str) -> Expression:
  ...
  if is_number(expression):
    ...
    return make_sum(derivative(addend(expression), variable_name), derivative(augend(expression), variable_name))
  elif is_product(expression):
    u = multiplier(expression)
    v = multiplicand(expression)
    return make_sum(make_product(u, derivative(v, variable_name)), make_product(v, derivative(u, variable_name)))
  else:
