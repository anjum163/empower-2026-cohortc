...
def money_to_string(money_value: Money) -> str:
    return f"{amount_in_cents(money_value) / 100:.2f} {currency_code(money_value)}"
...
#    Layer 2 uses the constructor and selectors from Layer 1.
#   The data crossing the barrier are the money objects, amounts and the currency codes through the constructors and selectors.
...
#    would not?
  #   Layer 0 and Layer 1 would change.
   #  Layer 2 and Layer 3 would not change.
