...
def add_money(first_amount: Money, second_amount: Money) -> Money:
  ...
  if currency_code(first_amount) != currency_code(second_amount):
    raise ValueError("Currency codes differ.")
  total_cents = amount_in_cents(first_amount) + amount_in_cents(second_amount)
  return make_money(total_cents, currency_code(first_amount))
...
def money_to_string(money_value: Money) -> str:
  ...
  cents = amount_in_cents(money_value) % 100
  return f"{dollars}.{cents:02d} {currency_code(money_value)}"
